import 'package:flutter/material.dart';

extension ThemeColors on Colors {
  // Background
  static var bg = const Color(0xFFFAF8F9);
  // Caption Text
  static var caption1 = const Color(0xFFA1BCF6);
  static var caption2 = const Color(0xFFB5B3B6);
  // Plain Circle Icons
  static var pearl = const Color(0xFFF3F3F7);
  static var darkBlue = const Color(0xFF444E6C);
  // Colored Circle Icons
  static var blue = const Color(0xFF3873FE);
  static var purple = const Color(0xFF8B77EC);
  static var pink = const Color(0xFFF365B6);
  static var orange = const Color(0xFFFEC558);
  static var green = const Color(0xFF5DCED1);
  static var sand = const Color(0xFFBEACAD);
}
